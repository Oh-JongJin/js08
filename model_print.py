...
class Tf_model():
    ...
    def inference(self, epoch, left_range, right_range, distance, cv_img, camera, azimuth):
        ...
        # visibility print
        print(f"{azimuth} visibility : ", visibility)

        # 이미지 저장
        self.result_save(result_list, azimuth, epoch, visibility)
    ...
    def result_save(self, dip_list: list, azimuth: str, epoch: str, visibility):
        """ 추론값과 현재 시정, 타겟 이미지들을 저장"""

        save_path = os.path.join(f'{JS08Settings.get("target_csv_path")}_images/')
        ...
        for distance, predict_idx, cv_img in dip_list[::-1]:
            ...
            if int(epoch[-2:]) % 5 == 00:
                if predict_idx == "n":
                    ...
                    image_path = os.path.join(no_target_save_path, f"{epoch}_{azimuth}_{distance}_{predict_idx}_{visibility}.png")
                else:
                    image_path = os.path.join(yes_target_save_path, f"{epoch}_{azimuth}_{distance}_{predict_idx}_{visibility}.png")
