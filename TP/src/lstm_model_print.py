...
class LSTM_model():
    ...
    def load_model(self):
        ...
        # model_path = "./saved_model_20230601165734/my_model"
    ...
    def predict_visibility(self, epoch, input_df):
        ...
        w2 = WindowGenerator(train_df=data_df, input_width=data_df.shape[0]-1, label_width=1, shift=1,
                    label_columns=['prev'])
        ...
        # inputs = example_inputs[None,:,:]
        # print("example_inputs", example_inputs.shape)
        # input_data = tf.constant(input_data)
        # input_data = input_data[np.newaxis, np.newaxis, :]
        prediction = self.lstm_model(example_inputs)
        prediction = (prediction.numpy() * self.prev_std  + self.prev_mean)[0][-1][0]
        return prediction
